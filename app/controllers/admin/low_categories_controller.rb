class Admin::LowCategoriesController < Admin::ApplicationController
  before_action :authorize_catalog_low_category

  def togglevisible
    @low_category = LowCategory.find(params[:id])
    @low_category.toggle(:visible)
    @low_category.save
    render 'admin/low_categories/togglevisible.js.coffee'
  end

  def change_visible
    val = params[:invisible].presence ? false : true
    LowCategory.where({id: params[:low_category_ids]}).update_all({visible: val})
    redirect_to :back
  end

  def sort
    LowCategory.sort(params[:low_category])
    render :nothing => true
  end
  ...
  def load_low_categories
    @low_categories ||= low_category_scope.to_a
  end
  ...
  def low_category_scope
    LowCategory.where(tenant_id: Tenant.current_id).order(:position)
  end
  ...
  def apply_redirect(notice = '')
    if params[:apply].present?
      redirect_to [:edit, :admin, @low_category], notice: notice
    else
      redirect_to [:admin, :low_categories], notice: notice
    end
  end

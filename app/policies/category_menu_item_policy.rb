class CategoryMenuItemPolicy < ApplicationPolicy
  class Scope < Scope
    def resolve
    ...
    def user_has_rights?
      user.authorization_profile_policies.try(:include?, "#{tenant_prefix}CategoryMenuItemPolicy")
    end
  end

  def user_has_rights?(tenant = current_tenant)
    prefix = tenant.url_scope.camelize
    user.authorization_profile_policies.try(:include?, "#{prefix}CategoryMenuItemPolicy")
  end
end
